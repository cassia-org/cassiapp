...
WineContext::WineContext(std::filesystem::path pRuntimePath, std::filesystem::path pPrefixPath)
        : runtimePath{std::move(pRuntimePath)}, prefixPath{std::move(pPrefixPath)},
          envVars{"WINEPREFIX=" + (prefixPath / "pfx").string(), "HOME=" + (prefixPath / "home").string(), "LD_LIBRARY_PATH=" + (runtimePath / "lib").string(), "PATH=" + (runtimePath / "bin").string(), "WINELOADER=" + (runtimePath / "arm64-v8a/bin/wine").string()},
          serverProcess{runtimePath / "arm64-v8a/bin/wineserver", {"--foreground", "--persistent"}, envVars} {
...
Process WineContext::Launch(std::string exe, std::vector<std::string> args, std::vector<std::string> pEnvVars) {
    ...
    return Process{runtimePath / "arm64-v8a/bin/wine", args, pEnvVars};
}

WineContext::~WineContext() {
    Launch("wineboot.exe", {"--end-session", "--shutdown"}).WaitForExit();
    Process{runtimePath / "arm64-v8a/bin/wineserver", {"--kill"}, envVars}.WaitForExit();
}
}
