 ...
 * Copyright © 2024 Cassia Team, Skyline Team and Contributors (https://github.com/skyline-emu/)
 */

package cassia.app.provider
...
class DocumentsProvider : DocumentsProvider() {
    private val baseDirectory = File(CassiaApplication.instance.filesDir.canonicalPath)
    private val applicationName = CassiaApplication.instance.applicationInfo.loadLabel(CassiaApplication.instance.packageManager).toString()
    ...
    private fun isIgnoredDocument(documentId: String?): Boolean {
        val file = getFile(documentId!!)
    ...
    override fun queryChildDocuments(parentDocumentId: String?, projection: Array<out String>?, sortOrder: String?): Cursor {
        ...
        for (file in parent.listFiles()!!)
            if (!isIgnoredDocument(getDocumentId(file)))
